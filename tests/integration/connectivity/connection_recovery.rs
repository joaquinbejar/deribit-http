...
async fn test_connection_persistence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    debug!("Testing {} requests with {}ms intervals", num_requests, request_interval.as_millis());
    ...
    for i in 0..num_requests {
        ...
        match result {
            Ok(time) => {
                ...
                debug!("Persistence request #{} succeeded in {:?}, time: {}", i + 1, elapsed, time);
            }
            Err(e) => {
                failed_requests += 1;
                debug!("Persistence request #{} failed in {:?}: {:?}", i + 1, elapsed, e);
            }
        }
    ...
    let avg_timing = if !request_timings.is_empty() {
        Duration::from_nanos(request_timings.iter().map(|d| d.as_nanos()).sum::<u128>() / request_timings.len() as u128)
    } else {
        Duration::from_secs(0)
    };
    
    let min_timing = request_timings.iter().min().unwrap_or(&Duration::from_secs(0));
    let max_timing = request_timings.iter().max().unwrap_or(&Duration::from_secs(0));
    
    info!("Connection persistence results:");
    info!("  Successful requests: {}/{}", successful_requests, num_requests);
    info!("  Failed requests: {}", failed_requests);
    info!("  Timing - avg: {:?}, min: {:?}, max: {:?}", avg_timing, min_timing, max_timing);
    ...
    assert!(success_rate >= 0.5, "Success rate should be at least 50% for connection persistence");
    
    // Timing should be reasonably consistent if connection is reused
    if successful_requests > 1 {
        let timing_variance = max_timing.as_millis() as f64 / min_timing.as_millis().max(1) as f64;
...
async fn test_retry_mechanism() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for attempt in 0..max_retries {
        ...
        match result {
            Ok(time) => {
                info!("Retry attempt #{} succeeded in {:?}, time: {}", attempt + 1, elapsed, time);
                // Success - break retry loop
                break;
            }
            Err(e) => {
                info!("Retry attempt #{} failed in {:?}: {:?}", attempt + 1, elapsed, e);
    ...
    match initial_auth_result {
        Ok(_) => {
            ...
            match recovery_request {
                ...
                Err(e) => {
                    info!("Authentication may have expired, testing re-authentication: {:?}", e);
                    ...
                    match reauth_result {
                        Ok(_) => {
                            ...
                            match reauth_request {
                                ...
                                Err(e) => info!("Request failed even after re-authentication: {:?}", e),
                            }
                        }
                        Err(e) => {
...
async fn test_concurrent_recovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    debug!("Starting {} concurrent requests for recovery testing", num_concurrent);
    ...
    for i in 0..num_concurrent {
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..3 {
                ...
                debug!("Concurrent task {} request {} completed in {:?}", i + 1, j + 1, elapsed);
    ...
    for handle in handles {
        match handle.await {
            Ok((task_id, task_results)) => {
                debug!("Concurrent task {} completed with {} results", task_id, task_results.len());
                all_results.extend(task_results.into_iter().map(|(req_id, result, elapsed)| {
                    (task_id, req_id, result, elapsed)
                }));
            }
            Err(e) => {
    ...
    for (task_id, req_id, result, elapsed) in all_results {
        ...
        match result {
            Ok(time) => {
                successful_requests += 1;
                debug!("Task {} request {} succeeded in {:?}, time: {}", task_id, req_id, elapsed, time);
            }
            Err(e) => {
                failed_requests += 1;
                debug!("Task {} request {} failed in {:?}: {:?}", task_id, req_id, elapsed, e);
            }
        }
    }
    ...
    info!("  Successful: {} ({:.1}%)", successful_requests, success_rate * 100.0);
    info!("  Failed: {}", failed_requests);
    info!("  Average time: {:?}", avg_time);
    
    // Concurrent requests should have reasonable success rate
    assert!(success_rate >= 0.3, "Concurrent success rate should be at least 30%");
...
async fn test_graceful_degradation_recovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (timeout_duration, description) in timeout_configs {
        debug!("Testing {} timeout configuration: {:?}", description, timeout_duration);
        ...
        for i in 0..3 {
            ...
            match result {
                Ok(time) => {
                    ...
                    debug!("{} config request #{} succeeded in {:?}, time: {}", 
                           description, i + 1, elapsed, time);
                }
                Err(e) => {
                    config_failure_count += 1;
                    debug!("{} config request #{} failed in {:?}: {:?}", 
                           description, i + 1, elapsed, e);
                }
            }
        ...
        info!("{} configuration results: {}/3 successes ({:.1}%)", 
              description, config_success_count, config_success_rate * 100.0);
...
async fn test_state_consistency_after_recovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match auth_result {
        Ok(_) => {
            ...
            info!("Authentication state before recovery simulation: {}", is_authenticated_before);
            ...
            info!("Authentication state after recovery simulation: {}", is_authenticated_after);
            ...
            match auth_request_result {
                ...
                Err(e) => {
                    info!("Authenticated request failed - testing state recovery: {:?}", e);
                    
                    // Check if we need to re-authenticate
                    let current_auth_state = client.is_authenticated();
                    info!("Current authentication state: {}", current_auth_state);
                    ...
                    if !current_auth_state {
                        ...
                        match reauth_result {
                            Ok(_) => {
                                ...
                                info!("Final authentication state: {}", final_auth_state);
                                ...
                                match final_request {
                                    ...
                                    Err(e) => info!("Request failed even after state recovery: {:?}", e),
                                }
                            }
                            Err(e) => {
                                info!("Re-authentication failed: {:?}", e);
        ...
        Err(e) => {
            ...
            match (public_result_1, public_result_2) {
                ...
                (Err(e1), Err(e2)) => {
                    info!("Public endpoints consistently failing: {:?} -> {:?}", e1, e2);
                }
            }
        }
    }

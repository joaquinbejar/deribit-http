...
async fn test_connection_timeout_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        ...
        Ok(Err(e)) => {
            ...
            assert!(error_str.contains("timeout") || error_str.contains("network") || error_str.contains("connection"),
    ...
    assert!(elapsed < Duration::from_secs(2), "Timeout should be respected");
...
async fn test_invalid_host_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut config = HttpConfig::testnet();
    config.base_url = "https://invalid-host-that-does-not-exist.com".to_string();
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(error_str.contains("network") || 
                   ...
                   "Error should be network-related: {}", e);
        }
    }
...
async fn test_ssl_certificate_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        ...
        Err(e) => {
            warn!("SSL connection failed (might be due to auth issues): {:?}", e);
            // This might fail due to auth, but not due to SSL issues
            let error_str = e.to_string().to_lowercase();
            assert!(!error_str.contains("ssl") && !error_str.contains("certificate") && !error_str.contains("tls"),
                    "Error should not be SSL-related: {}", e);
        }
    }
...
async fn test_network_recovery_simulation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match auth_result {
        Ok(_) => {
            ...
            info!("Network stress test completed: {} successes, {} failures", success_count, failure_count);
...
async fn test_concurrent_connection_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..3 {
        ...
        let handle = tokio::spawn(async move {
            ...
            debug!("Concurrent request #{} completed: {:?}", i + 1, result.is_ok());
            result
        });
        handles.push(handle);
    }
    ...
    for (i, handle) in handles.into_iter().enumerate() {
        match handle.await {
            Ok(Ok(time)) => {
                success_count += 1;
                debug!("Concurrent request #{} succeeded with time: {}", i + 1, time);
            }
            Ok(Err(e)) => {
                failure_count += 1;
    ...
    info!("Concurrent requests completed: {} successes, {} failures", success_count, failure_count);
    
    // At least one request should complete (either success or controlled failure)
    assert!(success_count + failure_count >= 3, "All requests should complete");
...
async fn test_graceful_degradation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, timeout_duration) in timeout_configs.into_iter().enumerate() {
        debug!("Testing with timeout configuration #{}: {:?}", i + 1, timeout_duration);
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(elapsed <= timeout_duration + Duration::from_millis(1000),
    ...
    for (host, expected_error_type) in error_scenarios {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(!error_lower.contains("panic"), "Error should not mention panics");
                assert!(!error_lower.contains("unwrap"), "Error should not mention unwrap");
            }
        }

...
async fn test_progressive_timeout_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, timeout_duration) in timeout_durations.into_iter().enumerate() {
        ...
        match result {
            Ok(time) => {
                info!("Timeout #{} ({:?}) succeeded in {:?}, time: {}", 
                      i + 1, timeout_duration, elapsed, time);
            ...
            Err(e) => {
                info!("Timeout #{} ({:?}) failed in {:?}: {:?}", 
                      i + 1, timeout_duration, elapsed, e);
                ...
                assert!(error_str.contains("timeout") || 
                       ...
                       "Error should be timeout-related: {}", e);
            }
        }
...
async fn test_timeout_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..num_requests {
        ...
        match result {
            Ok(time) => {
                info!("Request #{} succeeded in {:?}, time: {}", i + 1, elapsed, time);
                assert!(elapsed <= timeout_duration + Duration::from_millis(500),
    ...
    info!("Timing analysis - min: {:?}, max: {:?}, avg: {:?}", min_timing, max_timing, avg_timing);
    ...
    assert!(timing_ratio < 10.0, "Timing should be reasonably consistent (ratio: {})", timing_ratio);
...
async fn test_timeout_under_load() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match results {
        Ok(results) => {
            ...
            info!("Load test results: {} successes, {} failures, avg time: {:?}", 
                  success_count, failure_count, avg_time);
            
            // At least some requests should complete
            assert!(success_count + failure_count == num_concurrent,
...
async fn test_timeout_escalation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (attempt, timeout_duration) in escalation_timeouts.into_iter().enumerate() {
        debug!("Escalation attempt #{} with timeout {:?}", attempt + 1, timeout_duration);
        ...
        match result {
            Ok(time) => {
                info!("Escalation attempt #{} succeeded in {:?}, time: {}", 
            ...
            Err(e) => {
                info!("Escalation attempt #{} failed in {:?}: {:?}", 
                      attempt + 1, elapsed, e);
...
async fn test_timeout_with_authentication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth_result = if let (Ok(client_id), Ok(client_secret)) = 
        (std::env::var("DERIBIT_CLIENT_ID"), std::env::var("DERIBIT_CLIENT_SECRET")) {
        client.authenticate_oauth2(&client_id, &client_secret).await
    } else if let (Ok(api_key), Ok(api_secret)) = 
        (std::env::var("DERIBIT_API_KEY"), std::env::var("DERIBIT_API_SECRET")) {
    ...
    match auth_result {
        Ok(token) => {
            ...
            assert!(auth_elapsed <= timeout_duration + Duration::from_secs(1),
            ...
            match request_result {
                Ok(summary) => {
                    ...
                    assert!(request_elapsed <= timeout_duration + Duration::from_secs(1),
                            "Authenticated request should respect timeout");
                }
                Err(e) => {
                    info!("Authenticated request failed in {:?}: {:?}", request_elapsed, e);
                    assert!(request_elapsed <= timeout_duration + Duration::from_secs(1),
        ...
        Err(e) => {
            ...
            match public_result {
                Ok(time) => {
                    info!("Public endpoint succeeded in {:?}, time: {}", public_elapsed, time);
                    assert!(public_elapsed <= timeout_duration + Duration::from_secs(1),
...
async fn test_timeout_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (timeout_duration, description) in edge_case_timeouts {
        debug!("Testing edge case: {} ({:?})", description, timeout_duration);
            ...
            Ok(client) => {
                ...
                match result {
                    Ok(time) => {
                        ...
                        assert!(elapsed <= timeout_duration + Duration::from_secs(1),
                                "Request should respect timeout for {}", description);
                    }
                    Err(e) => {
                        info!("{} failed in {:?}: {:?}", description, elapsed, e);
                        assert!(elapsed <= timeout_duration + Duration::from_secs(1),
                                "Failed request should respect timeout for {}", description);
                    }
                }
            }
            Err(e) => {

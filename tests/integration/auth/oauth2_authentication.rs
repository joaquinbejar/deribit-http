...
fn check_env_file() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !Path::new(".env").exists() {
        return Err("Missing .env file. Please create one with DERIBIT_CLIENT_ID and DERIBIT_CLIENT_SECRET".into());
    }
...
async fn test_oauth2_authentication_success() -> Result<(), Box<dyn std::error::Error>> {
    ...
    debug!("Attempting OAuth2 authentication with client_id: {}", client_id);
    ...
    match auth_result {
        Ok(token) => {
            ...
            assert!(account_summary.is_ok(), "Should be able to make authenticated requests after OAuth2 login");
...
async fn test_oauth2_authentication_invalid_credentials() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth_result = client.authenticate_oauth2(invalid_client_id, invalid_client_secret).await;
    ...
    match auth_result {
        ...
        Err(e) => {
            info!("OAuth2 authentication correctly failed with invalid credentials: {:?}", e);
...
async fn test_oauth2_token_renewal() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let first_token = client.authenticate_oauth2(&client_id, &client_secret).await?;
    ...
    let second_token = client.authenticate_oauth2(&client_id, &client_secret).await?;

...
async fn test_get_deposits_btc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("BTC deposits retrieved successfully, count: {}, total: {}", 
          deposits_response.data.len(), deposits_response.count);
    debug!("Deposits response: {:?}", deposits_response);
    
    // Validate deposits response structure
    assert!(deposits_response.count >= deposits_response.data.len() as u32, 
            "Count should be >= data length: {} >= {}", 
            deposits_response.count, deposits_response.data.len());
    
    // Validate individual deposits
    for (i, deposit) in deposits_response.data.iter().enumerate() {
        debug!("Validating deposit #{}: {}", i + 1, deposit.transaction_id);
        
        assert!(!deposit.address.is_empty(), "Deposit address should not be empty");
        assert!(deposit.amount > 0.0, "Deposit amount should be positive");
        assert_eq!(deposit.currency, "BTC", "Currency should be BTC");
        assert!(deposit.received_timestamp > 0, "Received timestamp should be positive");
        assert!(!deposit.state.is_empty(), "State should not be empty");
        assert!(!deposit.transaction_id.is_empty(), "Transaction ID should not be empty");
        assert!(deposit.updated_timestamp > 0, "Updated timestamp should be positive");
        
        // Validate state values
        let valid_states = ["completed", "pending", "cancelled", "rejected"];
        assert!(valid_states.iter().any(|&s| deposit.state.contains(s)),
                "State should be valid: {}", deposit.state);
        
        // Validate timestamps order
        assert!(deposit.updated_timestamp >= deposit.received_timestamp,
                "Updated timestamp should be >= received timestamp: {} >= {}",
                deposit.updated_timestamp, deposit.received_timestamp);
    }
...
async fn test_get_deposits_eth() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("ETH deposits retrieved successfully, count: {}, total: {}", 
          deposits_response.data.len(), deposits_response.count);
    debug!("Deposits response: {:?}", deposits_response);
    
    // Validate that all deposits are ETH-related
    for deposit in &deposits_response.data {
        assert_eq!(deposit.currency, "ETH", "All deposits should be ETH-related");
    }
...
async fn test_get_deposits_with_count() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deposits_response = client.get_deposits("BTC", Some(requested_count), None).await?;
    
    info!("Deposits with count retrieved successfully, count: {}, total: {}", 
          deposits_response.data.len(), deposits_response.count);
    debug!("Deposits response: {:?}", deposits_response);
    
    // Validate that we got at most the requested count
    assert!(deposits_response.data.len() <= requested_count as usize, 
            "Should not receive more than requested count: {} <= {}", 
            deposits_response.data.len(), requested_count);
    ...
    if first_page.data.len() >= 5 {
        ...
        for first_deposit in &first_page.data {
            for second_deposit in &second_page.data {
                assert_ne!(first_deposit.transaction_id, second_deposit.transaction_id, 
...
async fn test_deposits_data_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Deposits retrieved for validation, count: {}", deposits_response.data.len());
    
    for deposit in &deposits_response.data {
        debug!("Validating deposit: {} - {}", deposit.transaction_id, deposit.currency);
        ...
        assert!(valid_currencies.contains(&deposit.currency.as_str()),
                "Currency should be valid: {}", deposit.currency);
        
        // Validate state values
        let valid_states = ["completed", "pending", "cancelled", "rejected", "confirming"];
        assert!(valid_states.iter().any(|&s| deposit.state.contains(s)),
                "State should be valid: {}", deposit.state);
        
        // Validate address format (basic check)
        assert!(deposit.address.len() >= 10, "Address should be at least 10 characters: {}", deposit.address);
        
        // Validate transaction ID format (basic check)
        assert!(deposit.transaction_id.len() >= 10, "Transaction ID should be at least 10 characters: {}", deposit.transaction_id);
...
async fn test_deposits_multiple_currencies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for currency in &currencies {
        ...
        info!("{} deposits retrieved successfully, count: {}, total: {}", 
              currency, deposits_response.data.len(), deposits_response.count);
        
        // Validate that all deposits match the requested currency
        for deposit in &deposits_response.data {
            assert_eq!(deposit.currency, *currency, 
...
async fn test_deposits_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(count_diff <= 5, "Deposit count should be relatively stable (diff: {})", count_diff);
    
    // Check that common deposits have consistent data
    for dep1 in &deposits1.data {
        if let Some(dep2) = deposits2.data.iter().find(|d| d.transaction_id == dep1.transaction_id) {
            assert_eq!(dep1.address, dep2.address, "Address should be consistent for {}", dep1.transaction_id);
            assert_eq!(dep1.amount, dep2.amount, "Amount should be consistent for {}", dep1.transaction_id);
            assert_eq!(dep1.currency, dep2.currency, "Currency should be consistent for {}", dep1.transaction_id);
            assert_eq!(dep1.received_timestamp, dep2.received_timestamp, "Received timestamp should be consistent for {}", dep1.transaction_id);
            
            // State might change, but updated timestamp should be >= previous
            assert!(dep2.updated_timestamp >= dep1.updated_timestamp,
                    "Updated timestamp should not go backwards for {}: {} >= {}",
                    dep1.transaction_id, dep2.updated_timestamp, dep1.updated_timestamp);
        }
    }
...
async fn test_deposits_empty_result() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Deposits with high offset retrieved successfully, count: {}, total: {}", 
          deposits_response.data.len(), deposits_response.count);

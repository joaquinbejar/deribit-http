...
async fn test_get_withdrawals_btc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("BTC withdrawals retrieved successfully, count: {}, total: {}", 
          withdrawals_response.data.len(), withdrawals_response.count);
    debug!("Withdrawals response: {:?}", withdrawals_response);
    
    // Validate withdrawals response structure
    assert!(withdrawals_response.count >= withdrawals_response.data.len() as u32, 
            "Count should be >= data length: {} >= {}", 
            withdrawals_response.count, withdrawals_response.data.len());
    
    // Validate individual withdrawals
    for (i, withdrawal) in withdrawals_response.data.iter().enumerate() {
        debug!("Validating withdrawal #{}: {:?}", i + 1, withdrawal.transaction_id);
        
        assert!(!withdrawal.address.is_empty(), "Withdrawal address should not be empty");
        assert!(withdrawal.amount > 0.0, "Withdrawal amount should be positive");
        assert_eq!(withdrawal.currency, "BTC", "Currency should be BTC");
        assert!(withdrawal.created_timestamp > 0, "Created timestamp should be positive");
        assert!(!withdrawal.state.is_empty(), "State should not be empty");
        assert!(withdrawal.transaction_id.is_some(), "Transaction ID should be present");
        if let Some(ref tx_id) = withdrawal.transaction_id {
            assert!(!tx_id.is_empty(), "Transaction ID should not be empty");
        }
        assert!(withdrawal.updated_timestamp > 0, "Updated timestamp should be positive");
        assert!(withdrawal.fee >= 0.0, "Fee should be non-negative");
        assert!(withdrawal.id > 0, "Withdrawal ID should be positive");
        
        // Validate state values
        let valid_states = ["completed", "pending", "cancelled", "rejected", "confirming", "unconfirmed"];
        assert!(valid_states.iter().any(|&s| withdrawal.state.contains(s)),
                "State should be valid: {}", withdrawal.state);
        
        // Validate timestamps order
        assert!(withdrawal.updated_timestamp >= withdrawal.created_timestamp,
                "Updated timestamp should be >= created timestamp: {} >= {}",
                withdrawal.updated_timestamp, withdrawal.created_timestamp);
        
        // Validate priority
        assert!(withdrawal.priority >= 0.0, "Priority should be non-negative");
    }
...
async fn test_get_withdrawals_eth() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("ETH withdrawals retrieved successfully, count: {}, total: {}", 
          withdrawals_response.data.len(), withdrawals_response.count);
    debug!("Withdrawals response: {:?}", withdrawals_response);
    
    // Validate that all withdrawals are ETH-related
    for withdrawal in &withdrawals_response.data {
        assert_eq!(withdrawal.currency, "ETH", "All withdrawals should be ETH-related");
    }
...
async fn test_get_withdrawals_with_count() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let withdrawals_response = client.get_withdrawals("BTC", Some(requested_count), None).await?;
    
    info!("Withdrawals with count retrieved successfully, count: {}, total: {}", 
          withdrawals_response.data.len(), withdrawals_response.count);
    debug!("Withdrawals response: {:?}", withdrawals_response);
    
    // Validate that we got at most the requested count
    assert!(withdrawals_response.data.len() <= requested_count as usize, 
            "Should not receive more than requested count: {} <= {}", 
            withdrawals_response.data.len(), requested_count);
...
async fn test_get_withdrawals_with_offset() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("First page retrieved successfully, count: {}", first_page.data.len());
    ...
    if first_page.data.len() >= 5 {
        ...
        info!("Second page retrieved successfully, count: {}", second_page.data.len());
        ...
        for first_withdrawal in &first_page.data {
            for second_withdrawal in &second_page.data {
                assert_ne!(first_withdrawal.transaction_id, second_withdrawal.transaction_id, 
                          "Transaction IDs should not overlap between pages");
                assert_ne!(first_withdrawal.id, second_withdrawal.id, 
...
async fn test_withdrawals_data_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Withdrawals retrieved for validation, count: {}", withdrawals_response.data.len());
    
    for withdrawal in &withdrawals_response.data {
        debug!("Validating withdrawal: {:?} - {}", withdrawal.transaction_id, withdrawal.currency);
        
        // Validate required string fields
        assert!(!withdrawal.address.is_empty(), "Address should not be empty");
        assert!(!withdrawal.currency.is_empty(), "Currency should not be empty");
        assert!(!withdrawal.state.is_empty(), "State should not be empty");
        assert!(withdrawal.transaction_id.is_some(), "Transaction ID should be present");
        if let Some(ref tx_id) = withdrawal.transaction_id {
            assert!(!tx_id.is_empty(), "Transaction ID should not be empty");
        ...
        assert!(withdrawal.priority >= 0.0, "Priority should be non-negative");
        assert!(withdrawal.priority.is_finite(), "Priority should be finite");
        assert!(withdrawal.id > 0, "Withdrawal ID should be positive");
        assert!(withdrawal.created_timestamp > 0, "Created timestamp should be positive");
        assert!(withdrawal.updated_timestamp > 0, "Updated timestamp should be positive");
        ...
        assert!(valid_currencies.contains(&withdrawal.currency.as_str()),
                "Currency should be valid: {}", withdrawal.currency);
        
        // Validate state values
        let valid_states = ["completed", "pending", "cancelled", "rejected", "confirming", "unconfirmed", "requesting"];
        assert!(valid_states.iter().any(|&s| withdrawal.state.contains(s)),
                "State should be valid: {}", withdrawal.state);
        
        // Validate address format (basic check)
        assert!(withdrawal.address.len() >= 10, "Address should be at least 10 characters: {}", withdrawal.address);
        
        // Validate transaction ID format (basic check)
        if let Some(ref tx_id) = withdrawal.transaction_id {
            assert!(tx_id.len() >= 10, "Transaction ID should be at least 10 characters: {}", tx_id);
        }
...
async fn test_withdrawals_multiple_currencies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for currency in &currencies {
        ...
        info!("{} withdrawals retrieved successfully, count: {}, total: {}", 
              currency, withdrawals_response.data.len(), withdrawals_response.count);
        
        // Validate that all withdrawals match the requested currency
        for withdrawal in &withdrawals_response.data {
            assert_eq!(withdrawal.currency, *currency, 
...
async fn test_withdrawals_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(count_diff <= 5, "Withdrawal count should be relatively stable (diff: {})", count_diff);
    
    // Check that common withdrawals have consistent data
    for with1 in &withdrawals1.data {
        if let Some(with2) = withdrawals2.data.iter().find(|w| w.transaction_id == with1.transaction_id) {
            assert_eq!(with1.address, with2.address, "Address should be consistent for {:?}", with1.transaction_id);
            assert_eq!(with1.amount, with2.amount, "Amount should be consistent for {:?}", with1.transaction_id);
            assert_eq!(with1.currency, with2.currency, "Currency should be consistent for {:?}", with1.transaction_id);
            assert_eq!(with1.fee, with2.fee, "Fee should be consistent for {:?}", with1.transaction_id);
            assert_eq!(with1.id, with2.id, "ID should be consistent for {:?}", with1.transaction_id);
            assert_eq!(with1.created_timestamp, with2.created_timestamp, "Created timestamp should be consistent for {:?}", with1.transaction_id);
            
            // State might change, but updated timestamp should be >= previous
            assert!(with2.updated_timestamp >= with1.updated_timestamp,
                    "Updated timestamp should not go backwards for {:?}: {} >= {}",
                    with1.transaction_id, with2.updated_timestamp, with1.updated_timestamp);
        }
    }
...
async fn test_withdrawals_state_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Withdrawals retrieved for state analysis, count: {}", withdrawals_response.data.len());
    ...
    for (state, count) in &state_counts {
        ...
        let valid_states = ["completed", "pending", "cancelled", "rejected", "confirming", "unconfirmed", "requesting"];
        assert!(valid_states.iter().any(|&s| state.contains(s)),
    ...
    if !fees.is_empty() {
        ...
        info!("Fee analysis - min: {}, max: {}, avg: {}", min_fee, max_fee, avg_fee);
...
async fn test_withdrawals_empty_result() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Withdrawals with high offset retrieved successfully, count: {}, total: {}", 
          withdrawals_response.data.len(), withdrawals_response.count);

...
async fn test_get_positions_all() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("All positions retrieved successfully, count: {}", positions.len());
    debug!("Positions: {:?}", positions);
    
    // Validate positions structure
    for (i, position) in positions.iter().enumerate() {
        debug!("Validating position #{}: {}", i + 1, position.instrument_name);
        
        assert!(!position.instrument_name.is_empty(), "Instrument name should not be empty");
        assert!(!position.kind.is_empty(), "Kind should not be empty");
        assert!(!position.direction.is_empty(), "Direction should not be empty");
        assert!(position.size != 0.0 || position.size == 0.0, "Size should be a valid number");
        assert!(position.average_price >= 0.0, "Average price should be non-negative");
        assert!(position.mark_price >= 0.0, "Mark price should be non-negative");
        assert!(position.index_price >= 0.0, "Index price should be non-negative");
        assert!(position.initial_margin >= 0.0, "Initial margin should be non-negative");
        assert!(position.maintenance_margin >= 0.0, "Maintenance margin should be non-negative");
    }
...
async fn test_get_positions_btc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("BTC positions retrieved successfully, count: {}", positions.len());
    debug!("BTC positions: {:?}", positions);
    
    // Validate that all positions are BTC-related
    for position in &positions {
        assert!(position.instrument_name.starts_with("BTC"), "All positions should be BTC-related: {}", position.instrument_name);
    }
...
async fn test_get_positions_eth() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("ETH positions retrieved successfully, count: {}", positions.len());
    debug!("ETH positions: {:?}", positions);
    
    // Validate that all positions are ETH-related
    for position in &positions {
        assert!(position.instrument_name.starts_with("ETH"), "All positions should be ETH-related: {}", position.instrument_name);
    }
...
async fn test_get_positions_by_kind_future() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Future positions retrieved successfully, count: {}", positions.len());
    debug!("Future positions: {:?}", positions);
    
    // Validate that all positions are futures
    for position in &positions {
        assert_eq!(position.kind, "future", "All positions should be futures: {}", position.instrument_name);
    }
...
async fn test_get_positions_by_kind_option() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Option positions retrieved successfully, count: {}", positions.len());
    debug!("Option positions: {:?}", positions);
    
    // Validate that all positions are options
    for position in &positions {
        assert_eq!(position.kind, "option", "All positions should be options: {}", position.instrument_name);
    }
...
async fn test_get_positions_combined_filters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("BTC future positions retrieved successfully, count: {}", positions.len());
...
async fn test_positions_data_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Positions retrieved for validation, count: {}", positions.len());
    ...
    for position in &positions {
        ...
        assert!(position.average_price.is_finite() && position.average_price >= 0.0, "Average price should be a finite non-negative number");
        assert!(position.mark_price.is_finite() && position.mark_price >= 0.0, "Mark price should be a finite non-negative number");
        assert!(position.index_price.is_finite() && position.index_price >= 0.0, "Index price should be a finite non-negative number");
        assert!(position.settlement_price.is_finite() && position.settlement_price >= 0.0, "Settlement price should be a finite non-negative number");
        
        // Validate margin fields
        assert!(position.initial_margin.is_finite() && position.initial_margin >= 0.0, "Initial margin should be a finite non-negative number");
        assert!(position.maintenance_margin.is_finite() && position.maintenance_margin >= 0.0, "Maintenance margin should be a finite non-negative number");
        assert!(position.open_orders_margin.is_finite() && position.open_orders_margin >= 0.0, "Open orders margin should be a finite non-negative number");
        
        // Validate P&L fields
        assert!(position.total_profit_loss.is_finite(), "Total P&L should be a finite number");
        assert!(position.realized_profit_loss.is_finite(), "Realized P&L should be a finite number");
        assert!(position.floating_profit_loss.is_finite(), "Floating P&L should be a finite number");
        assert!(position.realized_funding.is_finite(), "Realized funding should be a finite number");
        
        // Validate size fields
        assert!(position.size_currency.is_finite(), "Size currency should be a finite number");
        
        // Validate delta
        assert!(position.delta.is_finite(), "Delta should be a finite number");
        
        // Validate direction values
        assert!(position.direction == "buy" || position.direction == "sell" || position.direction == "zero", 
                "Direction should be buy, sell, or zero: {}", position.direction);
        
        // Validate kind values
        assert!(position.kind == "future" || position.kind == "option", 
                "Kind should be future or option: {}", position.kind);
    }
...
async fn test_positions_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(count_diff <= 2, "Position count should be relatively stable (diff: {})", count_diff);
    
    // Check that common positions have consistent basic data
    for pos1 in &positions1 {
        if let Some(pos2) = positions2.iter().find(|p| p.instrument_name == pos1.instrument_name) {
            assert_eq!(pos1.kind, pos2.kind, "Kind should be consistent for {}", pos1.instrument_name);
            
            // Size might change due to trading, but should be relatively stable for most positions
            let size_diff = (pos1.size - pos2.size).abs();
            if size_diff > pos1.size.abs() * 0.1 { // Allow 10% change
                warn!("Large size change detected for {}: {} -> {} (diff: {})", 

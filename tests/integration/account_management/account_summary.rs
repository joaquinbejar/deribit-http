...
async fn test_get_account_summary_btc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(account_summary.balance >= 0.0, "Balance should be non-negative");
    assert!(account_summary.available_funds >= 0.0, "Available funds should be non-negative");
    assert!(account_summary.equity >= 0.0, "Equity should be non-negative");
    assert!(!account_summary.username.is_empty(), "Username should not be empty");
    assert!(!account_summary.email.is_empty(), "Email should not be empty");
    assert!(account_summary.id > 0, "Account ID should be positive");
...
async fn test_get_account_summary_extended() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(account_summary.total_pl != 0.0 || account_summary.total_pl == 0.0, "Total P&L should be a valid number");
    assert!(account_summary.fee_balance >= 0.0, "Fee balance should be non-negative");
...
async fn test_get_account_summary_invalid_currency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(e.to_string().contains("invalid") || e.to_string().contains("currency") || e.to_string().contains("not found"));
        }
    }
...
async fn test_account_summary_multiple_currencies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for currency in &currencies {
        ...
        assert_eq!(account_summary.currency, *currency, "Currency should match requested currency");
        assert!(account_summary.balance >= 0.0, "Balance should be non-negative for {}", currency);
        assert!(account_summary.available_funds >= 0.0, "Available funds should be non-negative for {}", currency);
        assert!(account_summary.equity >= 0.0, "Equity should be non-negative for {}", currency);
        assert!(!account_summary.username.is_empty(), "Username should not be empty for {}", currency);
        assert!(account_summary.id > 0, "Account ID should be positive for {}", currency);
...
async fn test_account_summary_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(summary1.currency, summary2.currency, "Currency should be consistent");
    assert_eq!(summary1.username, summary2.username, "Username should be consistent");
    assert_eq!(summary1.email, summary2.email, "Email should be consistent");
    assert_eq!(summary1.id, summary2.id, "Account ID should be consistent");
    assert_eq!(summary1.account_type, summary2.account_type, "Account type should be consistent");
    ...
    assert!(balance_diff < 0.01, "Balance should be relatively stable (diff: {})", balance_diff);

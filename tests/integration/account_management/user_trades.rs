...
async fn test_get_user_trades_btc_perpetual() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_trades = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, None, None, None).await?;
    
    info!("BTC-PERPETUAL user trades retrieved successfully, count: {}", user_trades.len());
    ...
    for (i, trade) in user_trades.iter().enumerate() {
        ...
        assert!(!trade.order_type.is_empty(), "Order type should not be empty");
        assert_eq!(trade.instrument_name, "BTC-PERPETUAL", "Instrument name should be BTC-PERPETUAL");
        ...
        assert!(!trade.fee_currency.is_empty(), "Fee currency should not be empty");
        ...
        assert!(trade.direction == "buy" || trade.direction == "sell", 
                "Direction should be buy or sell: {}", trade.direction);
        
        // Validate liquidity values
        assert!(trade.liquidity == "M" || trade.liquidity == "T", 
                "Liquidity should be M (maker) or T (taker): {}", trade.liquidity);
        
        // Validate tick direction
        assert!(trade.tick_direction >= -1 && trade.tick_direction <= 1, 
                "Tick direction should be -1, 0, or 1: {}", trade.tick_direction);
    }
...
async fn test_get_user_trades_eth_perpetual() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_trades = client.get_user_trades_by_instrument("ETH-PERPETUAL", None, None, None, None, None).await?;
    
    info!("ETH-PERPETUAL user trades retrieved successfully, count: {}", user_trades.len());
    debug!("User trades: {:?}", user_trades);
    
    // Validate that all trades are for ETH-PERPETUAL
    for trade in &user_trades {
        assert_eq!(trade.instrument_name, "ETH-PERPETUAL", "All trades should be for ETH-PERPETUAL");
    }
...
async fn test_get_user_trades_with_count() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_trades = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(requested_count), None, None).await?;
    
    info!("User trades with count retrieved successfully, count: {}", user_trades.len());
    debug!("User trades: {:?}", user_trades);
    
    // Validate that we got at most the requested count
    assert!(user_trades.len() <= requested_count as usize, 
            "Should not receive more than requested count: {} <= {}", 
            user_trades.len(), requested_count);
...
async fn test_get_user_trades_with_sequence_range() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let initial_trades = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(10), None, None).await?;
    ...
    let filtered_trades = client.get_user_trades_by_instrument("BTC-PERPETUAL", Some(min_seq), Some(max_seq), None, None, None).await?;
    
    info!("User trades with sequence range retrieved successfully, count: {}", filtered_trades.len());
    debug!("Filtered trades: {:?}", filtered_trades);
    
    // Validate that all trades are within the sequence range
    for trade in &filtered_trades {
        assert!(trade.trade_seq >= min_seq, 
                "Trade sequence should be >= start_seq: {} >= {}", 
                trade.trade_seq, min_seq);
        assert!(trade.trade_seq <= max_seq, 
...
async fn test_get_user_trades_include_old() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let trades_no_old = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(10), Some(false), None).await?;
    
    // Get trades including old
    debug!("Getting trades including old");
    let trades_with_old = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(10), Some(true), None).await?;
    
    info!("User trades retrieved - no old: {}, with old: {}", trades_no_old.len(), trades_with_old.len());
    
    // With old trades should have >= trades without old
    assert!(trades_with_old.len() >= trades_no_old.len(), 
...
async fn test_get_user_trades_sorting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let trades_asc = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(10), None, Some("asc")).await?;
    
    // Get trades with descending sorting
    debug!("Getting trades with descending sorting");
    let trades_desc = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(10), None, Some("desc")).await?;
    
    info!("User trades retrieved - asc: {}, desc: {}", trades_asc.len(), trades_desc.len());
    
    // Validate ascending order
    if trades_asc.len() > 1 {
        for i in 1..trades_asc.len() {
            assert!(trades_asc[i].trade_seq >= trades_asc[i-1].trade_seq, 
    ...
    if trades_desc.len() > 1 {
        for i in 1..trades_desc.len() {
            assert!(trades_desc[i].trade_seq <= trades_desc[i-1].trade_seq, 
...
async fn test_user_trades_data_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_trades = client.get_user_trades_by_instrument("BTC-PERPETUAL", None, None, Some(20), None, None).await?;
    
    info!("User trades retrieved for validation, count: {}", user_trades.len());
    
    for trade in &user_trades {
        debug!("Validating user trade: {} - {}", trade.trade_id, trade.instrument_name);
        
        // Validate required string fields
        assert!(!trade.trade_id.is_empty(), "Trade ID should not be empty");
        assert!(!trade.order_id.is_empty(), "Order ID should not be empty");
        assert!(!trade.order_type.is_empty(), "Order type should not be empty");
        assert!(!trade.instrument_name.is_empty(), "Instrument name should not be empty");
        assert!(!trade.direction.is_empty(), "Direction should not be empty");
        assert!(!trade.fee_currency.is_empty(), "Fee currency should not be empty");
        assert!(!trade.liquidity.is_empty(), "Liquidity should not be empty");
        assert!(!trade.label.is_empty(), "Label should not be empty");
        ...
        assert!(trade.index_price.is_finite(), "Index price should be finite");
        ...
        assert!(trade.direction == "buy" || trade.direction == "sell", 
                "Direction should be buy or sell: {}", trade.direction);
        assert!(trade.liquidity == "M" || trade.liquidity == "T", 
                "Liquidity should be M or T: {}", trade.liquidity);
        ...
        // Validate order type
        let valid_order_types = ["limit", "market", "stop_limit", "stop_market", "take_limit", "take_market"];
        assert!(valid_order_types.iter().any(|&t| trade.order_type.contains(t)),
                "Order type should be valid: {}", trade.order_type);
        
        // Validate self trade flag
        assert!(trade.self_trade || !trade.self_trade, "Self trade should be boolean");
        
        // Validate optional matching ID
        if let Some(ref matching_id) = trade.matching_id {
            assert!(!matching_id.is_empty(), "Matching ID should not be empty if present");
        }
    }
...
async fn test_user_trades_multiple_instruments() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for instrument in &instruments {
        ...
        let user_trades = client.get_user_trades_by_instrument(instrument, None, None, Some(5), None, None).await?;
        
        info!("{} user trades retrieved successfully, count: {}", instrument, user_trades.len());
        
        // Validate that all trades match the requested instrument
        for trade in &user_trades {
            assert_eq!(trade.instrument_name, *instrument, 

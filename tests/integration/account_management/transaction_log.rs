...
async fn test_get_transaction_log_btc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_log = client.get_transaction_log("BTC", None, None, None, None).await?;
    
    info!("BTC transaction log retrieved successfully, count: {}", transaction_log.logs.len());
    ...
    for (i, log_entry) in transaction_log.logs.iter().enumerate() {
        debug!("Validating transaction log entry #{}: {}", i + 1, log_entry.transaction_type);
        
        assert!(log_entry.id > 0, "Transaction ID should be positive");
        assert!(log_entry.user_id > 0, "User ID should be positive");
        assert!(!log_entry.username.is_empty(), "Username should not be empty");
        assert!(log_entry.user_seq > 0, "User sequence should be positive");
        assert!(!log_entry.transaction_type.is_empty(), "Transaction type should not be empty");
        assert!(log_entry.timestamp > 0, "Timestamp should be positive");
        assert_eq!(log_entry.currency, "BTC", "Currency should be BTC");
        assert!(log_entry.change.is_finite(), "Change should be a finite number");
        assert!(log_entry.cashflow.is_finite(), "Cashflow should be a finite number");
        assert!(log_entry.balance.is_finite(), "Balance should be a finite number");
        assert!(log_entry.equity.is_finite(), "Equity should be a finite number");
        assert!(!log_entry.side.is_empty(), "Side should not be empty");
        
        // Validate optional fields if present
        if let Some(commission) = log_entry.commission {
            assert!(commission.is_finite(), "Commission should be a finite number");
        }
        if let Some(price) = log_entry.price {
            assert!(price.is_finite() && price >= 0.0, "Price should be a finite non-negative number");
        }
        if let Some(position) = log_entry.position {
            assert!(position.is_finite(), "Position should be a finite number");
        }
...
async fn test_get_transaction_log_eth() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_log = client.get_transaction_log("ETH", None, None, None, None).await?;
    
    info!("ETH transaction log retrieved successfully, count: {}", transaction_log.logs.len());
    debug!("Transaction log: {:?}", transaction_log);
    
    // Validate that all entries are ETH-related
    for log_entry in &transaction_log.logs {
        assert_eq!(log_entry.currency, "ETH", "All entries should be ETH-related");
    }
...
async fn test_get_transaction_log_with_count() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_log = client.get_transaction_log("BTC", None, None, Some(requested_count), None).await?;
    
    info!("Transaction log with count retrieved successfully, count: {}", transaction_log.logs.len());
    debug!("Transaction log: {:?}", transaction_log);
    
    // Validate that we got at most the requested count
    assert!(transaction_log.logs.len() <= requested_count as usize, 
            "Should not receive more than requested count: {} <= {}", 
            transaction_log.logs.len(), requested_count);
...
async fn test_get_transaction_log_with_time_range() -> Result<(), Box<dyn std::error::Error>> {
    ...
    debug!("Getting transaction log for time range: {} to {}", thirty_days_ago, now);
    let transaction_log = client.get_transaction_log("BTC", Some(thirty_days_ago), Some(now), None, None).await?;
    
    info!("Transaction log with time range retrieved successfully, count: {}", transaction_log.logs.len());
    debug!("Transaction log: {:?}", transaction_log);
    
    // Validate that all entries are within the time range
    for log_entry in &transaction_log.logs {
        assert!(log_entry.timestamp >= thirty_days_ago, 
...
async fn test_get_transaction_log_pagination() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let first_page = client.get_transaction_log("BTC", None, None, Some(10), None).await?;
    
    info!("First page retrieved successfully, count: {}", first_page.logs.len());
    ...
    if let Some(continuation) = &first_page.continuation {
        ...
        let second_page = client.get_transaction_log("BTC", None, None, Some(10), Some(continuation)).await?;
        
        info!("Second page retrieved successfully, count: {}", second_page.logs.len());
        ...
        if !first_page.logs.is_empty() && !second_page.logs.is_empty() {
            ...
            if first_page_latest > second_page_earliest {
                debug!("Pages might have overlapping timestamps: {} > {}", first_page_latest, second_page_earliest);
            }
        }
    } else {
...
async fn test_transaction_log_data_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_log = client.get_transaction_log("BTC", None, None, Some(20), None).await?;
    
    info!("Transaction log retrieved for validation, count: {}", transaction_log.logs.len());
    
    for log_entry in &transaction_log.logs {
        debug!("Validating transaction log entry: {} - {}", log_entry.id, log_entry.transaction_type);
        ...
        assert!(!log_entry.transaction_type.is_empty(), "Transaction type should not be empty");
        assert!(log_entry.timestamp > 0, "Timestamp should be positive");
        assert!(!log_entry.currency.is_empty(), "Currency should not be empty");
        assert!(!log_entry.side.is_empty(), "Side should not be empty");
        ...
        let valid_types = ["trade", "settlement", "transfer", "deposit", "withdrawal", 
                          "commission", "funding", "delivery", "exercise", "assignment"];
        assert!(valid_types.iter().any(|&t| log_entry.transaction_type.contains(t)) || 
                log_entry.transaction_type.starts_with("other"),
                "Transaction type should be valid: {}", log_entry.transaction_type);
        
        // Validate side values
        assert!(log_entry.side == "buy" || log_entry.side == "sell" || log_entry.side == "none",
                "Side should be buy, sell, or none: {}", log_entry.side);
        
        // Validate currency values
        let valid_currencies = ["BTC", "ETH", "USDC", "USDT", "EURR"];
        assert!(valid_currencies.contains(&log_entry.currency.as_str()),
                "Currency should be valid: {}", log_entry.currency);
        
        // Validate optional string fields if present
        if let Some(ref instrument_name) = log_entry.instrument_name {
            assert!(!instrument_name.is_empty(), "Instrument name should not be empty if present");
        }
        if let Some(ref order_id) = log_entry.order_id {
            assert!(!order_id.is_empty(), "Order ID should not be empty if present");
        }
        if let Some(ref trade_id) = log_entry.trade_id {
            assert!(!trade_id.is_empty(), "Trade ID should not be empty if present");
        }
        if let Some(ref price_currency) = log_entry.price_currency {
            assert!(!price_currency.is_empty(), "Price currency should not be empty if present");
        }
        if let Some(ref user_role) = log_entry.user_role {
            assert!(!user_role.is_empty(), "User role should not be empty if present");
        }
    }
...
async fn test_transaction_log_multiple_currencies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for currency in &currencies {
        ...
        let transaction_log = client.get_transaction_log(currency, None, None, Some(5), None).await?;
        
        info!("{} transaction log retrieved successfully, count: {}", currency, transaction_log.logs.len());
        
        // Validate that all entries match the requested currency
        for log_entry in &transaction_log.logs {
            assert_eq!(log_entry.currency, *currency, 

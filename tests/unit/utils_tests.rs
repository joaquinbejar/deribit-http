...
mod utils_tests {
    ...
    fn test_get_tomorrow_deribit_format_basic() {
        ...
        assert!(day_part.chars().all(|c| c.is_ascii_digit()), "Day part should be digits: {}", day_part);
        assert!(month_part.chars().all(|c| c.is_ascii_uppercase()), "Month part should be uppercase: {}", month_part);
        assert!(year_part.chars().all(|c| c.is_ascii_digit()), "Year part should be digits: {}", year_part);
    }
    ...
    fn test_format_structure() {
        ...
        assert_eq!(result.len(), 7, "Result should be exactly 7 characters: {}", result);
    ...
    fn test_is_tomorrow_date() {
        ...
        assert_eq!(result, expected, "Result should match tomorrow's date in Deribit format");
    }
    ...
    fn test_month_abbreviations() {
        ...
        assert!(valid_months.contains(&month_part), 
                "Month abbreviation '{}' should be valid", month_part);
    }
    ...
    fn test_day_padding() {
        ...
        assert!(day >= 1 && day <= 31, "Day {} should be between 1 and 31", day);
    }
    ...
    fn test_get_tomorrow_deribit_format_performance() {
        ...
        assert!(duration.as_secs() < 1, "Function took too long: {:?}", duration);
    }

...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("🕐 Time range: {} to {} (last 24 hours)", start_timestamp, end_timestamp);

    // Test with BTC-PERPETUAL and 1-hour resolution
    match client.get_tradingview_chart_data("BTC-PERPETUAL", start_timestamp, end_timestamp, "60").await {
        Ok(chart_data) => {
            ...
            if !chart_data.ticks.is_empty() {
                ...
                for i in 0..candles_to_show {
                    ...
                    info!("   {}. Candle {} - O:{:.2} H:{:.2} L:{:.2} C:{:.2} V:{:.6}", 
                         i + 1, timestamp, open, high, low, close, volume);
                }
                
                if chart_data.ticks.len() > candles_to_show {
                    info!("   💡 Showing first {} of {} candles", candles_to_show, chart_data.ticks.len());
                }
                
                // Calculate price range and movement
                if let (Some(&first_close), Some(&last_close)) = (chart_data.close.first(), chart_data.close.last()) {
                ...
                // High/Low analysis
                let highest_price = chart_data.high.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    ...
    match client.get_tradingview_chart_data("BTC-PERPETUAL", start_timestamp, end_timestamp, "15").await {
        Ok(chart_data) => {
            info!("✅ Chart data for BTC-PERPETUAL (15m) retrieved successfully");
            info!("📊 15-minute Chart Status: {}", chart_data.status);
            info!("📈 15-minute Data Points: {} candles", chart_data.ticks.len());
            ...
            if !chart_data.ticks.is_empty() {
                ...
                if !price_movements.is_empty() {
                    let avg_volatility = price_movements.iter().sum::<f64>() / price_movements.len() as f64;
    ...
    match client.get_tradingview_chart_data("ETH-PERPETUAL", start_timestamp, end_timestamp, "30").await {
        Ok(chart_data) => {
    ...
    info!("🕐 Short time range: {} to {} (last 4 hours)", short_start_timestamp, end_timestamp);

    match client.get_tradingview_chart_data("BTC-PERPETUAL", short_start_timestamp, end_timestamp, "5").await {
        Ok(chart_data) => {
            info!("✅ Short-term chart data for BTC-PERPETUAL (5m) retrieved successfully");
            info!("📊 Short-term Chart Status: {}", chart_data.status);
            info!("📈 Short-term Data Points: {} candles", chart_data.ticks.len());
            ...
            if !chart_data.ticks.is_empty() {
                ...
                if recent_candles > 1 {
                    info!("   📈 Recent Price Action (last {} candles):", recent_candles);
        ...
        Err(e) => {
            warn!("⚠️ Get short-term chart data for BTC-PERPETUAL (5m) error: {}", e);
    ...
    let resolutions = vec![("1", "1 minute"), ("3", "3 minutes"), ("10", "10 minutes"), ("60", "1 hour")];
    ...
    for (resolution, description) in &resolutions {
        match client.get_tradingview_chart_data("BTC-PERPETUAL", test_start, end_timestamp, resolution).await {
            Ok(chart_data) => {
                info!("✅ {} resolution: {} candles (status: {})", 
                     description, chart_data.ticks.len(), chart_data.status);
            }
            Err(e) => {
    ...
    match client.get_tradingview_chart_data("BTC-29MAR24", start_timestamp, end_timestamp, "60").await {
        Ok(chart_data) => {
    ...
    match client.get_tradingview_chart_data("INVALID-INSTRUMENT", start_timestamp, end_timestamp, "60").await {
        Ok(chart_data) => {
            if chart_data.ticks.is_empty() {
                info!("✅ Empty chart data for invalid instrument (expected behavior)");
            } else {
                warn!("⚠️ Unexpected chart data found for invalid instrument: {} candles", chart_data.ticks.len());
            }
        }
        Err(e) => {
    ...
    match client.get_tradingview_chart_data("BTC-PERPETUAL", start_timestamp, end_timestamp, "7").await {
        Ok(chart_data) => {
            if chart_data.status == "ok" {
                info!("✅ Unusual resolution accepted: {} candles", chart_data.ticks.len());
            } else {
                info!("⚠️ Unusual resolution returned status: {}", chart_data.status);
            }
        }
        Err(e) => {
    ...
    info!("⏰ Resolution: Time interval for each candle (1, 3, 5, 10, 15, 30, 60, 120, 180, 360 minutes)");
    println!();

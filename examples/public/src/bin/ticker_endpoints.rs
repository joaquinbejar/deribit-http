...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.get_ticker("BTC-PERPETUAL").await {
        Ok(ticker) => {
            ...
            if let (Some(high), Some(low)) = (ticker.stats.high, ticker.stats.low) {
                ...
                info!("      📊 24h Range: ${:.2} ({:.2}%)", range, range_percentage);
                
                if let Some(last_price) = ticker.last_price {
                    let position_in_range = (last_price - low) / range * 100.0;
                    info!("      🎯 Current Position in Range: {:.1}%", position_in_range);
                }
            }
        }
        Err(e) => {
    ...
    match client.get_ticker("ETH-PERPETUAL").await {
        Ok(ticker) => {
            ...
            if let (Some(bid_price), Some(ask_price)) = 
                ...
                info!("   📈 ETH Best Bid: ${:.2} | 📉 Best Ask: ${:.2}", bid_price, ask_price);
            }
        ...
        Ok(instruments) => {
            if let Some(future_instrument) = instruments.first() {
                ...
                match client.get_ticker(future_name).await {
                    Ok(ticker) => {
                        info!("✅ Ticker data for BTC future {} retrieved successfully", future_name);
                        ...
                        info!("   💡 Future contracts have expiration dates and may trade at premium/discount to spot");
                    }
                    Err(e) => {
    ...
    match client.get_instruments("BTC", Some("spot"), Some(false)).await {
        Ok(instruments) => {
            if let Some(spot_instrument) = instruments.first() {
                ...
                match client.get_ticker(spot_name).await {
                    Ok(ticker) => {
                        info!("✅ Ticker data for BTC spot {} retrieved successfully", spot_name);
                    ...
                    Err(e) => {
                        ...
                        info!("💡 This may be expected if the spot instrument is not currently active");
                    }
                }
            } else {
        ...
        Err(e) => {
            info!("⚠️ Could not fetch BTC spot instruments for ticker testing: {}", e);
            info!("💡 This may be expected on testnet with limited instrument availability");
        }
    }
        ...
        Ok(instruments) => {
            if let Some(option_instrument) = instruments.first() {
                ...
                match client.get_ticker(option_name).await {
                    Ok(ticker) => {
                        info!("✅ Ticker data for BTC option {} retrieved successfully", option_name);

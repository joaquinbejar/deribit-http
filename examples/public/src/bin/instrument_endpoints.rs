...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.get_instrument("NON-EXISTENT-INSTRUMENT").await {
        Ok(instrument) => {
            warn!("⚠️ Unexpected success for non-existent instrument: {}", instrument.instrument_name);
        }
        Err(e) => {
        ...
        Ok(instruments) => {
            ...
            for instrument in &instruments {
                if instrument.instrument_name.contains("PERPETUAL") {
                    perpetuals_count += 1;
                } else if instrument.instrument_name.contains("-C") || instrument.instrument_name.contains("-P") {
                    options_count += 1;
                } else if instrument.instrument_name.len() > 10 && !instrument.instrument_name.contains("-") {
                    futures_count += 1;
                } else {
                    other_count += 1;
                }
            ...
            if instruments.len() > 5 {
                info!("💡 Showing first 5 of {} BTC instruments", instruments.len());
            }
        }
        Err(e) => {
    ...
    match client.get_instruments("BTC", Some("future"), Some(false)).await {
        Ok(instruments) => {
    ...
    match client.get_instruments("BTC", Some("option"), Some(false)).await {
        Ok(instruments) => {
    ...
    match client.get_instruments("ETH", None, Some(false)).await {
        Ok(instruments) => {
            ...
            if instruments.len() > 3 {
                info!("💡 Showing first 3 of {} ETH instruments", instruments.len());
            }
        }
        Err(e) => {
    ...
    match client.get_instruments("BTC", Some("future"), Some(true)).await {
        Ok(instruments) => {
    ...
    match client.get_instruments("INVALID", None, Some(false)).await {
        Ok(instruments) => {
            if instruments.is_empty() {
                ...
                warn!("⚠️ Unexpected instruments found for invalid currency: {}", instruments.len());
            }
        }
        Err(e) => {
    ...
    info!("💡 Tip: Use get_instruments() to discover available instruments, then get_instrument() for detailed info");
    info!("📊 These endpoints are essential for understanding available trading instruments");
    info!("🔗 Filter by currency, kind (future/option/spot), and expired status for targeted results");

...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("   Start: {} ({})", start_timestamp, "24 hours ago from reference point");
    ...
    match client.get_funding_rate_history("BTC-PERPETUAL", start_timestamp, end_timestamp).await {
        Ok(funding_history) => {
            ...
            for (i, rate_data) in funding_history.iter().take(5).enumerate() {
                info!("   {}. Timestamp: {} - Interest 8h: {:.8} - Interest 1h: {:.8}", 
                     i + 1,
                     rate_data.timestamp,
                     rate_data.interest_8h,
                     rate_data.interest_1h);
                info!("      Index Price: ${:.2} | Prev Index Price: ${:.2}",
                     rate_data.index_price,
                     rate_data.prev_index_price);
            }
            
            if funding_history.len() > 5 {
                info!("💡 Showing first 5 of {} funding rate records", funding_history.len());
            }
    ...
    match client.get_funding_rate_history("ETH-PERPETUAL", start_timestamp, end_timestamp).await {
        Ok(funding_history) => {
            info!("✅ Funding rate history for ETH-PERPETUAL retrieved successfully");
            info!("📊 Found {} ETH funding rate records", funding_history.len());
            
            for (i, rate_data) in funding_history.iter().take(3).enumerate() {
                info!("   ETH {}. Interest 8h: {:.8} at {}", 
                     i + 1,
                     rate_data.interest_8h,
                     rate_data.timestamp);
            }
    ...
    match client.get_funding_rate_value("BTC-PERPETUAL", start_timestamp, end_timestamp).await {
        Ok(funding_rate_value) => {
            info!("✅ Funding rate value for BTC-PERPETUAL retrieved successfully");
            info!("📈 Funding rate value for period: {:.10}", funding_rate_value);
    ...
    match client.get_funding_rate_value("BTC-PERPETUAL", short_start_timestamp, end_timestamp).await {
        Ok(funding_rate_value) => {
        ...
        Err(e) => {
            warn!("⚠️ Get funding rate value for BTC-PERPETUAL (8h) error: {}", e);
            info!("💡 This may be expected if data is not available for this specific range");
        }
    }

    // Also test with ETH-PERPETUAL
    match client.get_funding_rate_value("ETH-PERPETUAL", start_timestamp, end_timestamp).await {
        Ok(funding_rate_value) => {
            info!("✅ Funding rate value for ETH-PERPETUAL retrieved successfully");
            info!("📈 ETH funding rate value for period: {:.10}", funding_rate_value);
        }
        Err(e) => {
    ...
    info!("==============================");
    info!("📈 /public/get_funding_rate_history - Historical funding rates for PERPETUAL instruments");
    ...
    info!("💡 Tip: Both endpoints are essential for understanding perpetual contract funding mechanisms");
    info!("🔗 Use funding rate history for analysis and funding rate value for period calculations");
    info!("⚠️ Note: Both endpoints work only with PERPETUAL instruments (BTC-PERPETUAL, ETH-PERPETUAL, etc.)");
